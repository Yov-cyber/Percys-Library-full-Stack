...
import { validateComicIdParam } from "../middleware/security";

async function isPathSafe(filePath: string): Promise<boolean> {
  try {
    const [resolved, libResolved] = await Promise.all([
      fs.realpath(filePath),
      fs.realpath(config.libraryPath),
    ]);
...
comicsRouter.param("id", (req, res, next, id) => {
  req.params.id = id;
  return validateComicIdParam(req, res, next);
});
  ...
  asyncHandler(async (req, res) => {
    ...
    switch (op) {
      ...
      case "markCompleted": {
        ...
        await evaluateAchievements(ownerId);
        await prisma.pendingComic.deleteMany({
          where: { ownerId, comicId: { in: ids } },
        });
        break;
      }
      case "markUnread": {
      ...
      case "categoryAdd": {
        ...
        const updates = await Promise.all(
          targets
            .filter((c) => !parseCategories(c.categories).includes(value))
            .map((c) =>
              prisma.comic.update({
                where: { id: c.id },
                data: {
                  categories: mergeCategoryTag(c.categories, value),
      ...
      case "categoryRemove": {
        ...
        const updates = await Promise.all(
          targets
            .filter((c) => parseCategories(c.categories).includes(value))
            .map((c) => {
              const nextArr = parseCategories(c.categories).filter((x) => x !== value);
              return prisma.comic.update({
                where: { id: c.id },
                data: {
                  categories: serializeCategories(nextArr),
...
comicsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      categories: parseCategories(comic.categories),
...
comicsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const etag = `W/"page-${req.params.id}-${n}-${autoCrop ? "1" : "0"}-${quality}-${page.data.length}-${page.data.slice(0, 8).toString("hex")}"`;
    res.setHeader("ETag", etag);
    if (req.headers["if-none-match"] === etag) {
      return res.status(304).end();
    }
...
const zoomSchema = z.object({ zoom: z.number().min(0.8).max(4) });
...
comicsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const current = parseCategories(exists.categories);
    const nextArray =
      next && !current.includes(next) ? [...current, next] : current;
    await prisma.comic.update({
      where: { id: req.params.id },
      data: {
        category: next,
        categories: serializeCategories(nextArray),
      },
    });
    res.json({ ok: true });
  }),
...
comicsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await prisma.comic.update({
      where: { id: req.params.id },
      data: {
        categories: serializeCategories(next),
