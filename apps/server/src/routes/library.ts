...
import { computeContentHash, scanLibrary, registerComicPath } from "../services/scanner";
import { reindexOwnerComics } from "../services/fts";
...
function serializeComic(c: RawComicRow) {
  return {
    id: c.id,
    title: c.title,
    format: c.format,
...
function buildListWhere(
    ...
    where.AND = [
      ...(Array.isArray(where.AND) ? where.AND : []),
      {
        OR: [
          ...
          { bookmarks: { some: { note: { contains: q } } } },
        ],
      },
    ];
  }
  ...
  if (parsed.category) {
    // Match legacy primary slot OR JSON array column (`["Marvel", ...]`).
    where.AND = [
      ...(Array.isArray(where.AND) ? where.AND : []),
      {
        OR: [
          { category: parsed.category },
          { categories: { contains: categoryJsonContainsNeedle(parsed.category) } },
        ],
      },
    ];
  }
...
libraryRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    void reindexOwnerComics(ownerId).catch(() => undefined);
    res.json(result);
  }),
);
...
libraryRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await prisma.comic.findMany({
      where: { ownerId, contentHash: { not: null } },
      select: { contentHash: true, sizeBytes: true },
    });
    const existingSignatures = new Set(existing.map((c) => `${c.contentHash}:${c.sizeBytes.toString()}`));
    ...
    for (const file of files) {
      ...
      const contentHash = await computeContentHash(file.path, fmt);
      const key = contentHash ? `${contentHash}:${BigInt(file.size).toString()}` : "";
      if (!key) {
        accepted.push(file);
        continue;
      }
