...
const envSchema = z.object({
  ...
  DATABASE_URL: z
    .string({
      required_error:
        "DATABASE_URL is not set. Copy apps/server/.env.example to apps/server/.env and fill in the database connection string.",
    })
    .min(1)
    .refine((value) => /^postgres(ql)?:\/\//.test(value) || /^file:/.test(value), {
      message: "DATABASE_URL must be a postgres://... URL or a file:... (SQLite) URL",
    }),
  ...
  /**
   * Shared secret for signed owner tokens. Development can run without it
   * in single-user mode; production must set it to avoid trusting headers.
   */
