...
export async function computeContentHash(p: string, fmt: ComicFormat): Promise<string | null> {
  try {
    const hash = crypto.createHash("sha256");
    if (fmt === "folder") {
      const files = (await collectFolderImages(p)).sort((a, b) => a.localeCompare(b));
      for (const filePath of files) {
        hash.update(path.relative(p, filePath));
        await hashFileInto(hash, filePath);
      }
    } else {
      await hashFileInto(hash, p);
    }
    return hash.digest("hex");
  } catch (err) {
    console.warn(`[scanner] hash failed for ${p}:`, err instanceof Error ? err.message : "unknown error");
    return null;
  }
}
...
async function runScan(ownerId: string): Promise<ScanResult> {
  ...
  const candidates: { path: string; format: ComicFormat }[] = [];
  ...
  async function walk(dir: string) {
    ...
    for (const entry of entries) {
      ...
      try {
        if (entry.isDirectory()) {
          // Keep `_uploads` in scan because user-imported comics are stored
          // there. Skipping all "_" folders was deleting valid rows on the
          // next scan/startup (rows looked "missing" and got purged).
          if (name.startsWith("_") && name !== "_uploads") continue;
          const isFolder = await isFolderComic(fullPath);
          if (isFolder) {
            candidates.push({ path: fullPath, format: "folder" });
          } else {
            await walk(fullPath);
          }
          ...
          if (fmt) {
            candidates.push({ path: fullPath, format: fmt });
          }
        }
      } catch (err) {
  ...
  // Process registration candidates concurrently with a limit of 4
  ...
  async function worker() {
    while (activeIndex < candidates.length) {
      ...
      try {
        const result = await registerComicPath(ownerId, item.path, item.format);
        if (result === "added") added += 1;
        if (result !== "skipped") seen.add(item.path);
      } catch (err) {
        console.warn(`[scanner] failed to register ${item.path}:`, err);
      }
    }
  }

  const workers = [];
  for (let i = 0; i < Math.min(CONCURRENCY_LIMIT, candidates.length); i++) {
    workers.push(worker());
  }
...
export async function registerComicPath(
  ...
  const contentHash = await computeContentHash(p, fmt);
  const title = path.basename(p).replace(/\.(cbz|cbr|pdf|zip|rar)$/i, "");

  if (!existing) {
    if (contentHash) {
      const duplicate = await prisma.comic.findFirst({
        where: { ownerId, contentHash, sizeBytes },
        select: { id: true, path: true },
      });
      if (duplicate) {
        console.warn(`[scanner] duplicate content skipped: ${p} matches ${duplicate.path}`);
        return "skipped";
      }
    }
    await prisma.comic.create({
      data: { ownerId, path: p, title, format: fmt, pageCount, sizeBytes, contentHash },
    });
    return "added";
  }
