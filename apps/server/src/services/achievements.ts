...
export interface AchievementDef {
  ...
  secret?: boolean;
  /** Shown in the UI while locked so players know how to earn it. */
  unlockHint?: string;
...
function buildCatalog(): AchievementDef[] {
  ...
  const extras: AchievementDef[] = [
    ...
    },
    {
      ...
      description: "Racha de 7 días y 10 cómics completados.",
      group: "secret",
      tier: 3,
      secret: true,
      unlockHint: "Mantén una racha de 7 días y completa al menos 10 cómics.",
      check: (c) => c.longestStreak >= 7 && c.totalRead >= 10,
    },
    {
      id: "secret-renaissance",
      title: "Renacentista",
      description: "Completa cómics de 3 categorías y 2 formatos distintos.",
      group: "secret",
      tier: 3,
      secret: true,
      unlockHint: "Termina cómics etiquetados en 3 categorías y en 2 formatos (CBZ, PDF, etc.).",
      check: (c) => c.categoriesCompleted >= 3 && c.formatsCompleted >= 2,
    },
    {
      id: "secret-time-traveler",
      title: "Viajero del tiempo",
      description: "30 días distintos con lectura registrada.",
      group: "secret",
      tier: 3,
      secret: true,
      unlockHint: "Lee al menos una página en 30 días de calendario diferentes.",
      check: (c) => c.totalReadingDays >= 30,
    },
    {
      id: "secret-percy-disciple",
      title: "Discípulo de Percy",
      description: "50 cómics completados en tu biblioteca.",
      group: "secret",
      tier: 4,
      secret: true,
      unlockHint: "Marca 50 cómics como leídos por completo.",
      check: (c) => c.totalRead >= 50,
    },
    {
      id: "secret-percys-milestone",
      title: "Hito de Percy",
      description: "500 páginas leídas en total.",
      group: "secret",
      tier: 4,
      secret: true,
      unlockHint: "Acumula 500 páginas leídas (progreso + cómics terminados).",
      check: (c) => c.totalPages >= 500,
    },
    {
      id: "secret-night-owl",
      title: "Noctámbulo",
      description: "100 páginas en un solo día.",
      group: "secret",
      tier: 3,
      secret: true,
      unlockHint: "Supera 100 páginas leídas en un mismo día (tu récord diario).",
      check: (c) => c.bestDayPages >= 100,
    },
    {
      id: "secret-perfect-collector",
      title: "Coleccionista selecto",
      description: "10 cómics marcados como favoritos.",
      group: "secret",
      tier: 2,
      secret: true,
      unlockHint: "Añade 10 cómics a favoritos.",
      check: (c) => c.favorites >= 10,
    },
    {
      id: "secret-power-user",
      title: "Lector veterano",
      description: "Racha de 14 días, 1.000 páginas y 5 favoritos.",
      group: "secret",
      tier: 4,
      secret: true,
      unlockHint: "Combina racha de 14 días, 1.000 páginas totales y 5 favoritos.",
      check: (c) => c.longestStreak >= 14 && c.totalPages >= 1_000 && c.favorites >= 5,
    },
  ];
...
export async function computeContext(ownerId = "default"): Promise<AchievementCtx> {
  ...
  for (const c of completed) {
    if (c.category?.trim()) categoryTags.add(c.category.trim());
    for (const tag of parseCategories(c.categories)) categoryTags.add(tag);
  }
  const categoriesCompleted = categoryTags.size;
...
export async function evaluateAchievements(ownerId = "default"): Promise<{ id: string; unlocked: boolean }[]> {
  ...
  if (unlocks.length > 0) {
    ...
    if (String(config.databaseUrl).startsWith("file:")) {
      for (const row of unlocks) {
        try {
          await prisma.achievement.create({ data: row });
        } catch (err) {
      ...
      await prisma.achievement.createMany({
        data: unlocks,
        skipDuplicates: true,
      } as Parameters<typeof prisma.achievement.createMany>[0]);
    }
  }
  return results;
}
