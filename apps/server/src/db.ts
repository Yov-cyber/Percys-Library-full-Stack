...
/**
 * Prisma client extended with a thin retry shim on every query. When the
 * database has a transient connectivity blip (P1001 / P1017 / P2024 / network
 * reset) the extension retries the query before letting the error propagate.
 */
...
export async function ensureSettings(ownerId = "default") {
  ...
  if (String(config.databaseUrl).startsWith("file:")) {
    const existing = await rawPrisma.settings.findUnique({ where: { ownerId } });
    if (!existing) {
      try {
        await rawPrisma.settings.create({ data: { ownerId } });
      } catch (err) {
    ...
    await rawPrisma.settings.createMany({
      data: [{ ownerId }],
      skipDuplicates: true,
    } as Parameters<typeof rawPrisma.settings.createMany>[0]);
  }
 ...
 * before surfacing to the caller, so a momentary database connectivity
 * blip doesn't cascade into a 500 the user sees.
 */
 ...
 *
 * Use this anywhere a single-shot database outage shouldn't break the
 ...
 * single log line whenever connectivity flips up↔down. Used so a database
 * outage produces ONE alert at the top of the logs instead of hundreds of
 * per-request errors.
 *
